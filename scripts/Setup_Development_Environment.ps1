...
function OptionRequirements {
	...
	if ($response -eq "Y") {
		...
		if ($install -eq "Y") {
			...
			$process.WaitForExit()
			while ($true) {
				$response = Read-Host "Git installation completed? (Y/N)"
				if ($response -eq "Y") {
					break
				}
			}
		...
		if ($install -eq "Y") {
			...
			$process.WaitForExit()
			while ($true) {
				$response = Read-Host "Python 3.9 installation completed? (Y/N)"
				if ($response -eq "Y") {
					break
				}
			}
		...
		if ($install -eq "Y") {
			...
			$process.WaitForExit()
			while ($true) {
				$response = Read-Host "NASM installation completed? (Y/N)"
				if ($response -eq "Y") {
					break
				}
			}
	...
	if ($response -eq "Y") {
		...
		if ($install -eq "Y") {
			if (-not (Test-Path -Path $folderBootkitsPath)) {
				...
				Invoke-Expression -Command ".\edksetup.bat Rebuild"
				...
				Write-Host "Open a cmd and perform a sample build executing edksetup.bat and build in a cmd" -ForegroundColor Yellow
