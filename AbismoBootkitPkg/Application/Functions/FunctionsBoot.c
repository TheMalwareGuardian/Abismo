...
#include <Functions/FunctionsEfiSystemPartition.h>
...
{
    // Hello World
    Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - Hello World\r\n");
    ...
    Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - Locate file in EFI partition (%S)\r\n", ImagePath);
    Status = FunctionsEfiSystemPartition_LocateFileInEfiPartition(ImagePath, &DevicePath);

    // Failed
    if (EFI_ERROR(Status))
    {
        Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - FunctionsEfiSystemPartition_LocateFileInEfiPartition failed: %llx (%r).\r\n", Status, Status);
        return Status;
    }


    // Load image into memory
    Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - Load image into memory\r\n");
    ...
    if (EFI_ERROR(Status))
    {
        Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - gBS->LoadImage failed: %llx (%r).\r\n", Status, Status);
        return Status;
    }


    // Start loaded image
    Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - Start loaded image\r\n");
    ...
    if (EFI_ERROR(Status))
    {
        Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - gBS->StartImage failed: %llx (%r).\r\n", Status, Status);
        return Status;
    }


    // Bye
    Print(L"AbismoApplicationUEFI [FunctionsBoot.c] (LoadImage) - Bye\r\n");
    return EFI_SUCCESS;
}
